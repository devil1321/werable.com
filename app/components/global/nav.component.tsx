...
const Nav = () => {
    ...
    if(typeof window !== 'undefined'){
      const makeVisible = () =>{
        ...
        if(window.innerWidth < 768){
          ...
          menuWrapperRef.current.style.maxHeight = '100vv'
          overlayRef.current.style.display = 'none'
  ...
  const handleMenu = () =>{
    if(!menuWrapperRef.current.classList.contains('--open')){
      ...
      if(!isPlaying){
        ...
        gsap.fromTo('a',{opacity:0,top:50},{opacity:1,top:0,stagger:0.2,duration:1,delay:1,onComplete:()=>setIsPlaying(false)})
      ...
      if(!isPlaying){
        ...
        gsap.fromTo('a',{opacity:1},{opacity:0,stagger:0.2,duration:1})
  ...
  return (
    <div className='nav w-[100vw] absolute top-0 left-0 z-10'>
      <div ref={overlayRef} className="nav-overlay opacity-0 absolute top-0 left-0 w-[100%] h-[100vh]"></div>
      ...
      <div ref={menuWrapperRef} className="nav-menus-wrapper z-30 overflow-hidden md:overflow-visible absolute max-w-[100%] w-[100%] md:w-fit md:max-w-[100vw] bg-neutral-900/70 md:bg-transparent h-[195px] md:h-max rounded-md top-[150px] lg:top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col md:flex-row justify-between items-center">
        <div className="nav-menu relative -left-[10%] md:-left-[5%] lg:-left-[15%] xl:-left-[10%] -top-4 w-1/3 flex gap-5">
          <Link className="relative top-0 left-0 hover:underline text-white" href="/home">Home</Link>
          <Link className="relative top-0 left-0 hover:underline text-white translate-y-2" href="/products">Products</Link>
          <Link className="relative top-0 left-0 hover:underline text-white translate-y-4" href="/about">About Us</Link>
        </div>
        ...
        <div className="nav-menu relative -left-[25%] md:-left-[5%] lg:left-[5%] xl:left-[10%] -top-4 w-1/3 flex gap-5">
          <Link className="relative top-0 left-0 hover:underline text-white translate-y-6" href="/new-sale">New Sale</Link>
          <Link className="relative top-0 left-0 hover:underline text-white translate-y-4" href="/cart">Cart</Link>
          <Link className="relative top-0 left-0 hover:underline text-white translate-y-2" href="/favoruites">Favoruites</Link>
          <Link className="relative top-0 left-0 hover:underline text-white" href="/contact">Contact</Link>
        </div>
      </div>
    </div>
  )
