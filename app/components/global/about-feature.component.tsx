...
const AboutFeature:React.FC<AboutFeatureProps> = ({isLeft,img,title,paragraph,opacity,className}) => {
  ...
  return (
    <div ref={featureRef} className={`about-feature my-12 md:flex md:justify-center md:items-center ${className}`}>
      <Image ref={imageRef} className={`about-feature-image min-w-1/2 ${isLeft ? "order-1" : 'order-2'} relative ${isLeft ? 'md:left-[10%]' : 'md:-left-[10%]'} top-0`} src={img} alt='about-image' width={800} height={600} />
      <div style={{opacity:opacity}} ref={detailsRef} className={`about-feature-details min-w-1/2 bg-neutral-700 text-white relative top-0 px-12 z-10 py-[50px] ${isLeft ? 'md:-left-[15%]' : 'md:left-[15%]'} ${isLeft ? 'order-2' : "order-1"}`}>
