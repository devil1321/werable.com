...
const Carousel = () => {
  ...
  const handleActive = () =>{
    const buttons = document.querySelectorAll('.home-carousel-feature-controls div') as NodeListOf<HTMLDivElement>
    buttons.forEach((b:HTMLDivElement) => b.style.opacity = '1')
    buttons[count].style.opacity = '0.7'
  }
  ...
  return (
    <div className='home-carousel w-[100vw] h-[120vh] relative top-0 left-0'>
      <div className="home-carousel-controls absolute top-[45%] left-1/2 -translate-y-1/2 -translate-x-1/2 flex justify-between items-center z-10 w-[80%]">
        <div onMouseEnter={()=>handleControl(prevRef)} onMouseLeave={()=>handleControl(prevRef)} className="cursor-pointer transition-all hover:bg-white hover:opacity-30 home-carousel-prev rounded-full bg-neutral-300 opacity-10 p-3" onClick={()=>handlePrev()}>
        ...
        <div onMouseEnter={()=>handleControl(nextRef)} onMouseLeave={()=>handleControl(nextRef)} className="cursor-pointer transition-all hover:bg-white hover:opacity-30 home-carousel-next rounded-full bg-neutral-300 opacity-10 p-3" onClick={()=>handleNext()}>
      ...
      <div className="home-carousel-items-wrapper h-[100vh] w-[100vw] overflow-hidden">
      ...
      <div className="home-carousel-feature relative -top-[127px] z-10 left-0">
        <Image className="opacity-70" src="/assets/carousel-controls-bg-top.png" alt='feature-item' width={1920} height={300} />
        <div className="home-carousel-feature-controls absolute top-1/2 left-1/2 z-10 -translate-y-1/2 -translate-x-1/2 flex gap-5">
            <div onClick={()=>setCount(0)} className="hover:bg-white/70  cursor-pointer w-10 h-2 bg-white"></div>
            <div onClick={()=>setCount(1)} className="hover:bg-white/70  cursor-pointer w-10 h-2 bg-white"></div>
            <div onClick={()=>setCount(2)} className="hover:bg-white/70  cursor-pointer w-10 h-2 bg-white"></div>
            <div onClick={()=>setCount(3)} className="hover:bg-white/70  cursor-pointer w-10 h-2 bg-white"></div>
        </div>
      </div>
    </div>
  )
